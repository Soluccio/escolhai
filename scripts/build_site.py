...
t=(R/'template.html').read_text(encoding='utf-8'); block='\n'.join(card(p) for p in ps); (R/'index.html').write_text(re.sub(r'<!-- PRODUCTS_START -->.*?<!-- PRODUCTS_END -->','<!-- PRODUCTS_START -->\n'+block+'\n<!-- PRODUCTS_END -->',t,flags=re.S),encoding='utf-8');print(f'{len(ps)} produtos gerados')
