...
for tpl,out in [('template.html','index.html'),('produtos-template.html','produtos.html')]:
 text=(R/tpl).read_text(encoding='utf-8');text=re.sub(r'<!-- PRODUCTS_START -->.*?<!-- PRODUCTS_END -->','<!-- PRODUCTS_START -->\n'+block+'\n<!-- PRODUCTS_END -->',text,flags=re.S);(R/out).write_text(text,encoding='utf-8')
print(f'{len(ps)} produtos atualizados nas duas páginas')
